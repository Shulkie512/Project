...
- Filebeats detects changes and collects data to the file system
- Metricbeats is used to detect changes in system metrics, such as up time, remote logins and CPU usage.

### Using the Playbook
In order to use the playbook, you will need to have an Ansible control node already configured. Assuming you have such a control node provisioned: 

SSH into the control node and follow the steps below:
- Copy the [Elk YML](https://github.com/Shulkie512/Project/blob/main/ElkInstall.txt) file to `etc/ansible` directory.
- Update the `hosts` file to include...
  * Internal IP of your Web1 and Web2 machines under `webservers` in your hosts file
  * Add `ELK` to your `hosts` file and add the internal IP for the server
  * Specify in your playbook which server to run the file on
- Run the playbook, and navigate to `http://internalElkIP:5601/app/kibana` to check that the installation worked as expected.
